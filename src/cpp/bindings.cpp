...
void MatrixMultiply(const FunctionCallbackInfo <Value> &args) {
    Local<Array> m1 = Local<Array>::Cast(args[0]);
    Local<Array> m2 = Local<Array>::Cast(args[1]);

    double *m1Data = new double[4];
    m2->NumberValue(args.GetIsolate()->GetCurrentContext()).To(m1Data);

    std::cout << m1Data[0] << std::endl << m1Data[1] << std::endl << m1Data[2] << std::endl << m1Data[3] << std::endl << m1->Length();
    //args.GetReturnValue().Set(array);
}

void Initialize(Local<Object> exports) {
  NODE_SET_METHOD(exports, "MatrixMultiply", MatrixMultiply);
}
